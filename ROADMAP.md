...
### ✅ Completado
- **12 vistas funcionales**: Login, DashboardSuperAdmin, DashboardAdmin, DashboardEstudiante, Institución, Campañas, CampañaForm, CampañaDetalle, Estudiantes, EstudianteDetalle, MisCampañas, Perfil
...
- **Detalle de estudiante**: Perfil completo con información personal, grado, institución, estado y campañas en las que participa
- **Filtro de estudiantes extraído** a `utils/filters.js` (eliminada duplicación entre Estudiantes e Institución)
- **15 bugs críticos/altos corregidos** (role checks, router loops, http.put, auth guards, etc.)
...
- **UX**: Transiciones de página, ConfirmDialog, validación inline, toggle contraseña, loading bar, notificaciones Toast
- **Accesibilidad**: Skip-link, focus-visible, aria-current, aria-hidden en SVGs, roles ARIA, keyboard nav, prefers-reduced-motion, high contrast
- **Backend mock**: Todos los endpoints con auth, requireRole, validaciones, CRUD completo de campañas, detalle de persona
...
| B1.6 | **Gestión de usuarios** — Crear, editar, desactivar usuarios (estudiantes, administradores) | SUPERADMIN | No hay forma de dar de alta nuevos usuarios |
| B1.7 | **Gestión de instituciones** — Crear/editar instituciones desde el panel SuperAdmin | SUPERADMIN | Actualmente solo se ven, no se administran |
...
11. **T1.1–T1.3** — Código muerto + duplicación
12. **D1.6** — Animación de conteo en stats
13. **M1.2–M1.6** — Migración completa a backend real
